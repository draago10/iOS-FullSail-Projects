    ...
    func checkForWin(){
        if cardImageDataDuplicate.count == matchedCards.count{
            ...
            let action = UIAlertAction(title: "Ok", style: .default) { a in
                let gameData = GameData(timeToComplete: self.gameTimeDisplay.text!, tapsToComplete: self.moveCount, dateCompleted: Date())
                self.player?.gameData.append(gameData)
                print(self.player?.gameData.count)
            }
